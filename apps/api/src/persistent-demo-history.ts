...
const sources: OrderSource[] = ['INSTAGRAM', 'WHATSAPP', 'FACEBOOK', 'TIKTOK', 'WEBSITE', 'PHONE'];
...
export async function seedPersistentDemoHistory(repositories: Repositories) {
  ...
  const company =
    resources.find((resource) => resource.type === 'COMPANY' && resource.active) ??
  ...
  for (let index = 0; index < 30; index += 1) {
    ...
    const timeline: Order['timeline'] = [
      {
        ...
        message: `Order captured from ${sources[index % sources.length]!.toLowerCase()}.`,
      ...
      },
      {
        ...
        fromStatus: 'PENDING_CUSTOMER_CONFIRMATION',
        toStatus: 'CONFIRMED',
        occurredAt: iso(createdAt + 30 * 60 * 1000),
      },
      {
        id: `${orderId}_prepared`,
        actorType: 'USER',
        actorId: ownerId,
        actorName: 'Joe',
        action: 'order.status_changed',
        message: 'Order prepared and packed.',
        fromStatus: 'CONFIRMED',
        toStatus: 'PACKED',
        occurredAt: iso(createdAt + 90 * 60 * 1000),
      },
      {
        id: `${orderId}_ready`,
        actorType: 'USER',
        ...
        actorName: 'Joe',
        action: 'order.status_changed',
        message: 'Parcel ready for dispatch.',
        fromStatus: 'PACKED',
        toStatus: 'READY_FOR_DISPATCH',
        occurredAt: iso(assignedAt - 10 * 60 * 1000),
      },
      {
        id: `${orderId}_assigned`,
        actorType: 'USER',
        actorId: ownerId,
        actorName: 'Joe',
        action: 'delivery.assigned',
        message: `Assigned to ${company.name} for ${zone.name}.`,
        fromStatus: 'READY_FOR_DISPATCH',
        toStatus: 'ASSIGNED_TO_DELIVERY',
        occurredAt: iso(assignedAt),
      },
      {
        id: `${orderId}_out`,
        actorType: 'USER',
        ...
        action: 'delivery.out_for_delivery',
        message: 'Delivery route started.',
        fromStatus: 'ASSIGNED_TO_DELIVERY',
      ...
      },
      {
        ...
        toStatus: failureReason ? 'FAILED' : 'DELIVERED',
        occurredAt: iso(completedAt),
      },
      ...(refunded
        ? [
            {
              id: `${orderId}_refunded`,
    ...
    const order: Order = {
      ...
      currency: 'USD',
      totals: {
        itemsSubtotal: usd(itemsTotal),
        discount: usd(0),
        deliveryFee: zone.customerFee,
      ...
      notes: [
        {
          id: `${orderId}_note`,
          text: 'Persistent demonstration transaction with full source evidence.',
          authorId: ownerId,
          authorName: 'Joe',
    ...
    const attempts: DeliveryCase['attempts'] = retried
      ? [
          {
            id: `${deliveryId}_attempt_1`,
            number: 1,
            status: 'FAILED',
            scheduledAt: iso(assignedAt),
            startedAt: iso(assignedAt + 60 * 60 * 1000),
            completedAt: iso(assignedAt + 2 * 60 * 60 * 1000),
            failureReason: 'CUSTOMER_UNAVAILABLE',
            note: 'Customer requested a later retry.',
            commandId: `${deliveryId}-retry`,
            actorId: company.id,
          },
          {
            id: `${deliveryId}_attempt_2`,
            number: 2,
            status: 'DELIVERED',
            scheduledAt: iso(completedAt - 60 * 60 * 1000),
            startedAt: iso(completedAt - 30 * 60 * 1000),
            completedAt: iso(completedAt),
            note: 'Delivered on the agreed second attempt.',
            commandId: `${deliveryId}-done`,
            actorId: company.id,
          },
        ]
      : [
          {
            ...
            note: failureReason
              ? 'Reason recorded for owner follow-up.'
              : 'Delivered on first attempt.',
            commandId: `${deliveryId}-closed`,
            actorId: company.id,
          },
        ];
    const delivery: DeliveryCase = {
      ...
      resourceType: company.type,
      zoneId: zone.id,
      zoneName: zone.name,
      customerFee: zone.customerFee,
      businessCost: zone.businessCost,
      expectedCollection: usd(total),
      attempts,
      assignmentHistory: [
        {
          id: `${deliveryId}_assignment`,
          resourceId: company.id,
          resourceName: company.name,
          assignedBy: 'Joe',
          assignedAt: iso(assignedAt),
          reason: 'Scheduled historical dispatch',
        },
      ],
      createdAt: iso(assignedAt),
    ...
    if (!failureReason) {
      const sale: InventoryMovement = {
        ...
        sourceType: 'ORDER',
        sourceId: orderId,
        reason: `Delivered ${orderNumber}`,
        unitCost: selected.variant.currentUnitCost,
        idempotencyKey: `history:${orderId}:sale`,
      ...
      if (index % 13 !== 0) {
        ...
        if (payment.method === 'CASH') {
          ...
          const collection: CustodyMovement = {
            id: `cash_history_${sequence}`,
            tenantId,
            paymentId: payment.id,
            type: 'DRIVER_COLLECTION',
            amount: payment.amount,
            toHolderId: company.id,
            toHolderName: company.name,
            occurredAt: payment.occurredAt,
            actorId: company.id,
            note: `Cash collected for ${orderNumber}.`,
          };
          const handover: CustodyMovement = {
            id: `handover_history_${sequence}`,
            tenantId,
            paymentId: payment.id,
            reconciliationId: 'rec_history_cash',
            type: 'HANDOVER_ACCEPTED',
            amount: payment.amount,
            fromHolderId: company.id,
            fromHolderName: company.name,
            occurredAt: iso(completedAt + 86_400_000),
            actorId: ownerId,
      ...
      if (refunded) {
        ...
        const returnCase: ReturnCase = {
          ...
          note: 'Sellable item returned and restocked.',
          customerRequestChannel: 'WHATSAPP',
          customerRequestReference: `Demo chat ${sequence}`,
          items: [
            {
              orderLineId: lineId,
              productId: selected.product.id,
          ...
          refundAmount: usd(total),
          refundMethod: customer.preferredPaymentMethod,
          refundReference: `REF-${sequence}`,
          createdAt: iso(completedAt + 86_400_000),
          createdBy: ownerId,
          receivedAt: iso(resolvedAt),
          receivedBy: ownerId,
        ...
        await inventory.saveMovement({
          ...
          reason: `Sellable return from ${orderNumber}`,
          unitCost: selected.variant.currentUnitCost,
          idempotencyKey: `history:${orderId}:return`,
          createdAt: iso(resolvedAt),
          createdBy: ownerId,
        });
        await fulfillment.savePaymentEntry({
          id: 'pay_history_refund_019',
          tenantId,
          orderId,
          orderNumber,
          type: 'REFUND',
  ...
  if (cashPayments.length) {
    ...
    const reconciliation: Reconciliation = {
      ...
      remaining: usd(0),
      variance: usd(0),
      explanation: 'Historical COD manifest matched in full.',
      paymentIds: cashPayments.map((payment) => payment.id),
      createdAt: closedAt,
      createdBy: ownerId,
      approvedAt: closedAt,
      approvedBy: ownerId,
    ...
    const updated: Customer = {
      ...
      orderStats: {
        ...
        lifetimeSpendUsdMinor: completed.reduce(
          (sum, order) =>
            sum + (order.status === 'REFUNDED' ? 0 : order.totals.grandTotal.amountMinor),
          0,
        ),
